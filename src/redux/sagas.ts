  ...
  try {
    ...
    if (payload) {
      const response: ResponseGenerator = yield call(() =>
        Service.getQuestionCount(payload)
      );
...
function* getQuestionsSaga({ amount, category, difficulty, qtype }: any) {
  try {
    yield put(showQuestionLoader(true));
    const response: ResponseGenerator = yield call(() =>
      Service.getQuestions(amount, category, difficulty, qtype)
    );
...
function* getCustomQuestionSaga({ payload }: any) {
  try {
    yield put(showQuestionLoader(true));
    const response: IQuestion[] = yield call(() =>
      CustomQuizService.getCustomQuizQuestions(payload)
    );
...
export function* sagaWatcher() {
  ...
  yield takeEvery(
    QuizActionTypes.FETCH_CUSTOM_QUIZ_QUESTIONS,
    getCustomQuestionSaga
  );
