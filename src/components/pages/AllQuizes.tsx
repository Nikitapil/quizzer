...
export const AllQuizes = () => {
    ...
    if (!quizLoading && !allQuizes.length) {
        return (
            <main className='user-quizes__empty'>
                <h1 className='user-quizes__title'>There is no any quizes yet</h1>
            </main>
        )
    }
  return (
    <main className='user-quizes all-quizes container'>
    <h1 className='user-quizes__title'>All quizes from users</h1>
    <p className='all-quizes__notice'>There is only non private quizes from all users</p>
    {quizLoading && <Loader/>}
    <ul className='user-quizes__list'>
        {(allQuizes && !quizLoading) && allQuizes.map(quiz => {
