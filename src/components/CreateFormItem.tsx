...
export const CreateFormItem:FC<ICreateFormItem> = ({question, rightAnswer, wrongAnswers, index, onChangeQuestion, onChangeRightAnswer, onChangeWrongAnswer, onAddItem, onDeleteQuestion, onDeleteWrongAnswer}) => {
  ...
  return (
    <fieldset className='question-field'>
      {index > 2 && <button type="button" className='question-field-del' onClick={deleteQuestion}><FontAwesomeIcon icon={faXmark} /></button>}
        ...
        {wrongAnswers.map((answ, Windex) => {
            return (
                <WrongAnswer onDeleteAnswer={deleteWrongAnswer}  index={Windex} value={wrongAnswers[Windex]} onChangeValue={(val) => onChangeWrongAnswer(Windex, val)} key={Windex}/>
            )
        })}
